...
def main():
    ...
    xpoints.insert(0, xpoints[-1] - 400)
    xpoints.append(xpoints[1] + 400)
    rpoints.insert(0, rpoints[-1])
    rpoints.append(rpoints[1])
    gpoints.insert(0, gpoints[-1])
    gpoints.append(gpoints[1])
    bpoints.insert(0, bpoints[-1])
    bpoints.append(bpoints[1])
...
def output32bits(f, R, G, B):
    ...
    for i in range(NPOINTS):
        ...
        s = '0x%08x, ' % packinto32bits(
            boundcheck(int(R(x))), boundcheck(int(G(x))), boundcheck(int(B(x)))
        )
        f.write(s)
        if i % 20 == 0:
            f.write('\n')
