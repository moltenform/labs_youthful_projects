...
def pattern(s, asserts=False):
    ...
    for chr in s:
        seenchars[chr] = True
    if len(seenchars) != 2 or '0' not in seenchars:
        raise 'String must consist of 0s and one other char'
    otherchar = [key for key in seenchars if key != '0'][0]
    start = s.index(otherchar)
    end = s.rindex(otherchar)

    print(
        '%s is a %dbit number packed into bits %d to %d' %
        (otherchar, 1 + end - start, start, end)
    )
    if asserts:
        print('assert(%s < %d);' % (otherchar, 2**(len(s.replace('0', '')))))
    if start == 0: #all we need to do is shift it
        ...
        print(
            'Unpacking:\n %s %s = packed & 0x%x; //packed & 0b%s' %
            (type, otherchar, int(mask, 2), mask)
        )
    else: #need to shift and mask
        print('Packing:\n %s packed |= %s<<%d;' % (type, otherchar, len(s) - end - 1))
        mask = '0' * start + '1' * (len(s) - start)
        print(
            'Unpacking:\n %s %s = (packed & 0x%x)>>%d; //packed & 0b%s' %
            (type, otherchar, int(mask, 2), len(s) - end - 1, mask)
        )
