...
def showGitkAndReset():
    ...
    files.run(
        'git_commit_--no-verify_-m_(Temporary commit for diffing)'.split('_')
    ) # commit them so gitk shows it first
...
def gitpTop_Pack(projname=None, desc=None):
    ...
    with zipfile.ZipFile(fullfilename, 'w') as zip:
        ...
        gitpTop_Pack_AddManifest(
            zip, changes, root, tmproot, proj, shortdesc, longdesc, fullfilename
        )
    trace(f"complete. wrote to {fullfilename}")


def gitpTop_Pack_AddManifest(
    zip, changes, root, tmproot, proj, shortdesc, longdesc, fullfilename
):
    manifest = {}
    manifest['gitp'] = 1
    ...
    try:
        for i in range(1, 10):
            ...
            manifest[f'adjacentCommitstTime{i}'] = fnLastLine(
                'git_log_-' + str(i) + '_--pretty=%cD'
            )
            manifest[f'adjacentCommitstUnix{i}'] = fnLastLine(
                'git_log_-' + str(i) + '_--pretty=%ct'
            )
            manifest[f'adjacentCommitsMsg{i}'] = getCommitMsgFromCommitId(
                manifest[f'adjacentCommits{i}']
            )
...
def gitpTop_Pack_GrabFiles(root, tmproot, zip, changes):
    ...
    grabBaseVersionsOfFiles(
        root, tmproot, changes,
        lambda longpath, shortpath: addToZipWithSuffix(zip, longpath, shortpath, '000')
    )
    grabWorkingVersionsOfFiles(
        root, tmproot, changes,
        lambda longpath, shortpath: addToZipWithSuffix(zip, longpath, shortpath, '001')
    )
...
def gitpTop_Pack_AddPatch(root, tmproot, zip):
    ...
    with ChangeCurrentDirectory(tmproot) as cd:
        ...
        assertTrue(
            files.exists(patchPath) and files.getSize(patchPath) > 0, 'not found or empty',
            patchPath
        )
...
def moveAllChangesLeftToRight(root, tmproot, basis):
    with ChangeCurrentDirectory(tmproot) as cd:
        ...
        assertGitPacket(
            not areThereUnstagedFiles(), f'please have no unstaged files in {tmproot}'
        )
...
def gitpTop_SlowCopyToAltRepo():
    ...
    files.runRsync(
        src, dest, deleteExisting=True, excludeRelative=['.git', 'node_modules', 'dist']
    )
...
def gitpTop_Apply_ApplyAndCheck(fullpathtozip=None, alwaysToRoot=False):
    ...
    with zip:
        ...
        if shasMatch(manifest["basis"], currentCommitId()):
            ...
            trace(
                f'This patch was developed against commmit id {manifest["basis"]}. We recommend that you go '
            ...
            for path in manifest['added']:
                if not files.exists(path):
                    ...
                    if not compareWithFileInZip(zip, path, '001'):
                        warnings.append(
                            f'patch intended to add path {path} and it is not the same'
                        )
            for path in manifest['modified']:
                if not files.exists(path):
                    ...
                    if expectPerfectApply and not compareWithFileInZip(zip, path, '001'):
                        warnings.append(
                            f'patch intended to modify path {path} and it is not the same'
                        )
    if warnings:
        trace('warnings:' + '\n'.join(warnings))
...
def gitpTop_UpdateBasis():
    ...
    steps.append(
        f'Now both root and tmproot are ready. you can optionally push changes ' +
