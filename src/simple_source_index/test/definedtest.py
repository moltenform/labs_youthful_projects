...
def main():
    ...
    if os.path.exists('ssip.db'):
        os.unlink('ssip.db')
    if os.path.exists('ssip_test.db'):
        os.unlink('ssip_test.db')
    if os.path.exists('ssip_test.db'):
        raise 'need to delete db'
    if os.path.exists(sbinary):
        os.unlink(sbinary)
    if os.path.exists(sfile1):
        os.unlink(sfile1)
    if os.path.exists(sfile1):
        raise 'need to delete tempfile'
...
def tests():
    ...
    assertContains(
        bk.txt, 'contrived.cpp:16: a wantstring { also twice w'
    ) #don't show same line x3
