...
class AudioOptionManager(object):
    ...
    def get(self, optionid):
        ...
        # mark that we've looked up this option
        self.mapIdToOptionAndVariable[optionid][2] = True 
        if opt.vals == 'Str':
            return var.get()
        elif opt.vals == 'Bool':
            ...
            if n < opt.lbound:
                raise AssertionError(
                    'number for %s needs to be at least %s' % (opt.caption, opt.lbound)
                )
            if n > opt.rbound:
                raise AssertionError(
                    'number for %s needs to be smaller than %s' % (opt.caption, opt.rbound)
                )
            return n
        else:
            v = var.get()
...
class WindowAudioOptions(object):
    def __init__(self, top, callbackOnClose=None):
        ...
        self.options.addOption(
            'bitdepth',
            'Output quality:', [('16-bit', 16), ('24-bit', 24)],
            initialval=defaultBitdepth
        )
        ...
        self.options.addOption(
            'PatchesTakePrecedence', 'Patches have priority over soundfonts', 'Bool'
        )
        if sys.platform.startswith('win'):
            self.options.addOption(
                'useold', 'Use old timidity version (requires .cfg and/or .pat)', 'Bool'
            )
            self.options.addOption(
                'bypasstimidity', 'Bypass all settings and send MIDI to OS', 'Bool'
            )

        # room/reverb settings
        self.options.addOption(
            'delay', 'Stereo delay type:', [('(default)', None), ('Off', 'd'),
                                            ('Left delay', 'l'), ('Right delay', 'r'),
                                            ('Rotate', 'b')]
        )
        self.options.addOption('delayAmount', 'Stereo delay (milliseconds):', 'Int')
        self.options.addOption(
            'chorus', 'Chorus type:', [('(default)', None), ('Disabled', 'd'), ('Enabled', 'n'),
                                       ('Surround', 's')]
        )
        self.options.addOption(
            'chorusAmount', 'Chorus strength %:', 'Int', lbound=0, rbound=100
        )
        self.options.addOption(
            'reverb', 'Reverb type:', [('(default)', None), ('Disabled', 'd'),
                                       ('Older Reverb', 'n'), ('Older Reverb, Global', 'g'),
                                       ('Newer Reverb', 'f'), ('Newer Reverb, Global', 'G')]
        )
        self.options.addOption(
            'reverbAmount', 'Reverb strength %:', 'Int', lbound=0, rbound=100
        )
        self.options.addOption('reverbPseudo', 'Global pseudo reverb (1-5000):', 'Int')
        self.options.addOption(
            'lpf', 'Low pass filter:', [('(default)', None), ('Off', 'd'), ('12dB / oct', 'c'),
                                        ('24dB / oct', 'm')]
        )
        self.options.addOption(
            'fastdecay', 'Fast decay', [('(default)', None), ('Off', False), ('On', True)]
        )

        # advanced settings
        self.options.addOption(
            'antialias', 'Antialias', [('(default)', None), ('Off', False), ('On', True)]
        )
        ...
        self.options.addOption(
            'controlratio', 'Control ratio, smaller for better quality', 'Int'
        )
    ...
    def getOptionsListImpl(self, includeRenderOptions=False):
        ret = []
        allowNotSeen = dict(
            useold=1, bitdepth=1, AdditionalCfg=1, bypasstimidity=1, PatchesTakePrecedence=1
        )
        ...
        if self.getUseOldTimidity():
            ...
            if self.options.get('antialias') is not None:
                ret.append(
                    '--anti-alias' if self.options.get('antialias') else '--no-anti-alias'
                )
            if self.options.get('fastdecay') is not None:
                ret.append(
                    '--fast-decay' if self.options.get('fastdecay') else '--no-fast-decay'
                )
            if self.options.get('maxpolyphony'):
            ...
            if self.options.get('volcurve') is not None:
                ret.append(
                    '--volume-curve=%f' % fromPercentage('volcurve', 1.661, truncate=False)
                )
    ...
    def createUI(self, top):
        ...
        Label(frameTop, text='Settings remain in effect while this dialog is open.' +
              ' ' * 15).pack(pady=5)
