...
class App(object):
    def __init__(self, root):
        ...
        self.sliderTime = Scale(
            frameDecogroup, orient=HORIZONTAL, resolution=0.1, width=5, from_=0, to_=0
        )
        self.sliderTime.pack(side=TOP, fill=X)

        frameBtns = Frame(frameDecogroup)
        self.btnPlay = pack(
            Button(frameBtns, image=self.icon0, text='Play', command=self.onBtnPlay),
            side=LEFT,
            padx=4
        )
        self.btnPause = pack(
            Button(frameBtns, image=self.icon1, text='Pause', command=self.onBtnPause),
            side=LEFT,
            padx=4
        )
        self.btnStop = pack(
            Button(
                frameBtns, image=self.icon2, text='Stop', command=self.onBtnStop, relief=SUNKEN
            ),
            side=LEFT,
            padx=4
        )
        pack(
            Button(frameBtns, text='Choose SoundFont...', command=self.openSoundfontWindow),
            side=LEFT,
            padx=(70, 4)
        )
        pack(
            Button(frameBtns, text='Save Wav...', command=self.onBtnSaveWave),
            side=LEFT,
            padx=4
        )
        ...
        self.currentSoundfont = [
            midirender_soundfont.getDefaultSoundfont()
        ] # an array so we can pass it as a reference.
        self.create_menubar(root)
        self.clearModifications()
        self.player = midirender_playback.BMidiRenderPlayback(
            self.playCallbackToggleButton, self.playCallbackGetSlider,
            self.playCallbackSetSlider
        )
    ...
    def create_menubar(self, root):
        ...
        root.bind('<Alt-F4>', lambda _: root.quit)
        ...
        menuFile.add_command(
            label="Open MIDI", command=self.menu_openMidi, underline=0, accelerator='Ctrl+O'
        )
        menuFile.add_separator()
        menuFile.add_command(
            label="Modify MIDI Events...", command=self.menuModifyRawMidi, underline=0
        )
        menuFile.add_separator()
        menuFile.add_command(
            label="Save Changes From Mixer...",
        ...
        menuAudio.add_command(
            label="Change Tempo...", command=self.menu_changeTempo, underline=0
        )
        menuAudio.add_command(
            label="Transpose Pitch...", command=self.menu_changeTranspose, underline=0
        )
        menuAudio.add_command(
            label="Copy Options String", command=self.menuCopyAudioOptsString, underline=1
        )
        menuAudio.add_separator()
        menuAudio.add_command(
            label="Save Wav", command=self.onBtnSaveWave, underline=5, accelerator='Ctrl+R'
        )
        menuAudio.add_separator()
        menuAudio.add_command(
            label="Choose SoundFont...",
        ...
        menuView.add_command(
            label="Mixer", command=self.openMixerView, underline=0, accelerator='Ctrl+M'
        )
        menuView.add_command(
            label="Console Output", command=self.menu_openConsoleWindow, underline=0
        )
        menuView.add_separator()
        menuView.add_command(
            label="SoundFont Information Tool",
            command=self.menu_soundFontInfoTool,
            underline=0
        )
        menuView.add_separator()
        menuView.add_checkbutton(
            label="Show Durations in score",
        ...
        menuHelp.add_command(
            label='Documentation', underline=0, command=(lambda: openOnlineDocs())
        )
    ...
    def menu_openMidi(self, evt=None):
        ...
        try:
            ...
            midirender_util.alert(
                'Could not load midi: exception %s' % str(e),
    ...
    def loadMidiObj(self, newmidi):
        ...
        def addLabel(text, y, x, isButton=False):
            ...
            if (x, y + 1) not in self.gridwidgets:
                ...
                if isButton:
                    ...
                    btn.config(
                        command=midirender_util.Callable(self.onBtnChangeInstrument, y, btn)
                    )
                    btn.pack(anchor='w', fill=BOTH)
                    btn[
                        'disabledforeground'
                    ] = 'black' # means that when it is disabled, looks just like a label. sweet.
                    thewidget = btn
        ...
        self.player.load(
            max(1.0, overallLengthSeconds + 1.0)
        ) # "loading" will also set position to 0.0
        ...
        for rownum in range(len(self.objMidi.tracks)):
            ...
            if len(instarray) == 0:
                ...
                instname = str(instarray[0]
                              ) + ' (' + bmidilib.getInstrumentName(instarray[0]) + ')'
            if channame == '10':
            ...
            if (rownum, 1) not in self.gridbuttons:
                btn = Button(
                    self.frameGrid,
                    text='Score',
                    command=midirender_util.Callable(self.openScoreView, rownum)
                )
            ...
            if (rownum, 2) not in self.gridbuttons:
                btn = Button(
                    self.frameGrid,
                    text='List',
                    command=midirender_util.Callable(self.openListView, rownum)
                )
    ...
    def onBtnChangeInstrument(self, y, btn):
        ...
        dlg = midirender_choose_midi_voice.ChooseMidiInstrumentDialog(
            self.frameGrid, 'Choose Instrument', min(theEvt.data, 127)
        )
        midiNumber = dlg.result
        if midiNumber is None:
            return
    ...
    def onBtnSaveWave(self, e=None):
        ...
        filename = midirender_util.ask_savefile(
            title="Create Wav File", types=['.wav|Wav file']
        )
        if not filename:
            return
    ...
    def buildCfg(self):
        ...
        if filename.endswith('.cfg'):
            ...
            if self.audioOptsWindow and self.audioOptsWindow.getUseOldTimidity():
                ...
                if justname.lower(
                ) != 'timidity.cfg' and os.path.exists(path + os.sep + 'timidity.cfg'):
                    midirender_util.alert(
        ...
        if self.soundfontWindow is not None:
            strCfg += '\n' + self.soundfontWindow.getCfgResults(
                self.audioOptsWindow and self.audioOptsWindow.getUseOldTimidity()
            )
    ...
    def menu_changeTranspose(self, e=None):
        ...
        res = midirender_util.ask_float(
            strPrompt, default=default, min=-24.1, max=24.1, title='Transpose'
        )
    ...
    def openScoreView(self, n):
        ...
        top = Toplevel()
        window = scoreview.ScoreViewWindow(
            top, n, self.objMidi.tracks[n], self.objMidi.ticksPerQuarterNote, opts
        )
    ...
    def getParamsForTimidity(self, bRenderWav):
        ...
        if self.transposePitches is not None and (
            not self.audioOptsWindow or not self.audioOptsWindow.getUseOldTimidity()
        ):
    ...
    def onBtnPlay(self, e=None):
        ...
        if params is not None:
            self.player.actionPlay(
                self.buildModifiedMidi(), params, self.buildCfg(), directoryForOldTimidity,
                not bypassTimidity
            )
