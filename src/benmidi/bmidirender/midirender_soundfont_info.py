...
class BSoundFontInformation(tkSimpleDialog.Dialog):
    ...
    def body(self, top):
        ...
        self.lblCurrentSf = pack(
            Label(frameLoadSf, text='Soundfont:none.sf2'), side=LEFT, padx=5
        )
        ...
        for name in self.labelFieldNames:
            self.lblInfoFields[name] = pack(
                Label(frameInfo, text=name + ': '), side=TOP, anchor='nw'
            )
        ...
        self.lbVoices = pack(
            midirender_util.ScrolledListbox(frameVoices, width=45, height=9), side=TOP
        )
        ...
        if self.bSelectMode:
            Label(frPrevBtns, text='Preview voice:').pack(side=LEFT)
        else:
            Label(frPrevBtns, text='Preview Soundfont:').pack(side=LEFT)
        ...
        if not self.bSelectMode:
            ...
            self.varPrevVoiceOnly = IntVar()
            self.varPrevVoiceOnly.set(1)
            Checkbutton(
                framePreview, var=self.varPrevVoiceOnly, text='Preview the selected voice.'
    ...
    def loadSf(self, filename):
        ...
        if self.bSelectMode:
            self.lbVoices.selection_set(0)

    def loadMid(self):
        filename = midirender_util.ask_openfile(
            title="Choose midi song", types=['.mid|Midi'], initialfolder=sampleMidiPath
        )
        if not filename:
            return
    ...
    def previewSfStart(self):
        if self.player != None and self.player.isPlaying:
            return #don't play while something is already playing.
...
class SFInfoException(Exception):
    pass
...
class SoundFontInfo(object):
    ...
    def __repr__(self):
        s = 'name: %s\n date: %s\n author: %s\n product: %s\n copyright: %s\n comment: %s' % (
            self.name, self.date, self.author, self.product, self.copyright, self.comment
        )
