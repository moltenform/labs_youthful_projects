...
class App(object):
    ...
    def __init__(self, root):
        ...
        frameBtns.pack()
        self.btnPlay = Button(
            frameMain, command=self.playMidi, width=20, height=3, text='Play', font=tupfontbtn
        )
        ...
        self.btnSaveMid = Button(
            frameMain,
            command=self.saveMidi,
            width=20,
            height=3,
            text='Save mid',
            font=tupfontbtn
        ...
        #have a cached of stripped examples. then, when changing, check against this dict.
        self.cachedExamples = {} 
        self.exampleText = []
        ...
        self.txtMain.bind('<Control-a>', lambda _event: selectAll(self.txtMain), '+')
    ...
    def create_menubar(self, root):
        ...
        menuFile.add_command(
            label="Play", command=self.playMidi, underline=0, accelerator='Ctrl+space'
        )
        menuFile.add_command(
            label="Save mid", command=self.saveMidi, underline=0, accelerator='Ctrl+S'
        )
        menuFile.add_separator()
        ...
        if txt:
            ...
            def addExample(nExample, exampleName, exampleCode, sType):
                ...
                menuExamples.add_command(
                    label=exampleName,
                    command=midiscript_util.Callable(self.loadExample, nExample, sType)
                )
        ...
        menuHelp.add_command(
            label='About',
            command=(
                lambda: midiscript_util.
                alert('tunescript, by Ben Fisher 2009\n\n', 'benmidi tunescript')
            )
        )
    ...
    def createMidiFile(self):
        if self.mode == 'tune':
            return self.createMidiFileTunescript(self.getText())
        elif self.mode == 'code':
            return self.createMidiFileBuilder(self.getText())
        else:
            raise 'Bad mode' + self.mode
    ...
    def setModeTune(self):
        self.mode = 'tune'
        self.status('Switched to tunescript mode')
        self.loadExample(self.firstExampleTune, 'tune')

    def setModeCode(self):
        self.mode = 'code'
        self.status('Switched to code mode')
        self.loadExample(self.firstExampleCode, 'code')
