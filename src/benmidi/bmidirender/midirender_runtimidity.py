...
class RenderTimidityMidiPlayer(bmidiplay.TimidityMidiPlayer):
    ...
    def _startPlayback(self, strMidiPath):
        ...
        try:
            ...
            raise bmidiplay.PlayMidiException(
...
def isTimidityInstalled():
    import subprocess
    try:
        #will just display version info, etc. if suceeds
        process = subprocess.Popen(['timidity'], stdout=subprocess.PIPE
                                  ) 
    except EnvironmentError as e:
        return False
    return True
...
if __name__ == '__main__':
    ...
    def start(top):
        def start():
            global mmplayer
            mmplayer.playSynchronous(
                midirender_util.bmidirenderdirectory + '\\..\\midis\\tempotest.mid'
            )
