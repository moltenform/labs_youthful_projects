...
class ScoreViewFrame(Frame):
    ...
    def __init__(self, master, trackdata, ticksPerQtrNote, opts):
        ...
        if 'prefer_flats' in opts and opts['prefer_flats']:
            self.prefersharpflat = 'b'

        if 'clefspath' in opts and opts['clefspath'] != None:
            self.clefspath = opts['clefspath']
        else:
            self.clefspath = defaultClefsPath
        ...
        if len(self.trackdata.notelist) and self.scaleTicksToPixels(
            self.trackdata.notelist[0].time
        ) > self.defaultWidth + 700:
            percentage = (
                self.scaleTicksToPixels(self.trackdata.notelist[0].time) -
                (self.defaultWidth / 2.0)
            ) / float(self.completeWidth)
    ...
    def calculateCompleteWidth(self):
        ...
        if len(self.trackdata.notelist) == 0:
            self.lastTick = 100
        else:
            self.lastTick = self.trackdata.notelist[-1].endEvt.time + 100
    ...
    def drawBackground(self, opts):
        ...
        if 'show_barlines' in self.opts and self.opts['show_barlines']:
            for xTicks in range(
                4 * self.ticksPerQtrNote, self.lastTick, 4 * self.ticksPerQtrNote
            ):
                #doesn't start at 0, to not draw first line
                drawbarline(xTicks)
    ...
    def drawNote(self, notenumber, starttimecode, endtimecode):
        ...
        posy, sharpflat = self.getnoteposition(
            notenumber, self.shiftnotes, self.prefersharpflat
        )
        ...
        if 'show_stems' in self.opts and self.opts['show_stems']:
            if posy >= 0:
                drawstem(xPixels + 3, fy, 1) #treble clef, stem up
            else:
                drawstem(xPixels, fy, -1)
