...
class BMidiRenderPlayback(object):
    ...
    def actionPlay(
        self, midiCopy, arParams, strConfig, directoryForOldTimidity, bRenderPreview=False
    ):
        ...
        if sys.platform == 'win32' and not bRenderPreview:
            ...
            self.midiPlayerObject.playMidiObject(
                midiCopy, False, fromMs=self.currentTime * 1000
            )

        else:
            #play with timidity. we'll have to cut the midi ourselves.
            bmiditools.makeVeryRoughTimeExcerpt(
                midiCopy,
                float(self.currentTime) / float(self.lengthTime)
            )
