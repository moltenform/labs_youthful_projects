...
class BMidiBuilder(object):
        ...
        try:
            ...
            if n is None:
                raise BuilderException(
                    'Unable to interpret note name , use the format "c#" or "c#4".'
                )
        assert duration > 0
        assert n >= 0
        assert velocity > 0 and velocity <= 127
        self.notes.append(
            SimpleNote(n, self.currentTime, duration, velocity, percussion=percussion)
        )
        self.currentTime += duration

    def rewind(self, timestep=None):
        if timestep == None:
            timestep = self.notes[-1].duration #if given with no args, goes back
    ...
    def insertMidiEvent(self, evt, bUseCurrentTime=True):
        ...
        if evt.channel != None:
            raise BuilderException(
                "Must leave channel blank, it will be assigned correctly upon build time."
            )
    ...
    def insertPitchBendEvent(self, number): #pitch bend, from -100 to 100, accepts floats.
        if number < -100 or number > 100:
            raise BuilderException('Pitch bend: Out of range, -100 to 100')
    ...
    def setInstrument(self, instrument):
        ...
        try:
            ...
            if found == None:
                raise BuilderException(
                    'Unable to find instrument %s, look at the end of "bmidiconstants.py" to see list of instruments, or find an online chart of midi instrument numbers.'
                )
    ...
    def build_note_events(self, channel): #returns a list of events, not yet a track
        if len(self.notes) == 0:
            raise BuilderException('Cannot save empty file, has to contain some notes.')
        evtlist = []
...
def build_midi(builderObjects):
    ...
    for i in range(len(builderObjects)):
        channel = i + 1
        noteEventList = builderObjects[i].build_note_events(
            channel
        ) #returns a list of events, not yet a track
