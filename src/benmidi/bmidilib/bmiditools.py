...
def getTrackInformation(trackObject, searchFor):
    ...
    if bSeeTracknames:
        currentName = searchFor['TRACKNAME'] #what to return if no names found
    if bSeeChannels:
        currentChannels = {}
    if bSeeInstruments:
        currentInstruments = {}
    ...
    res = {}
    if bSeeTracknames:
        res['TRACKNAME'] = currentName
    if bSeeInstruments:
        res['INSTRUMENTS'] = list(currentInstruments.keys())
    return res
...
class BMidiSecondsLength(object):
    def __init__(self, midiObject):
        ...
        for track in midiObject.tracks:
            tempoChanges.extend(((evt.time, bmidilib.dataToTempo(evt.data))
                                 for evt in track.events if evt.type == 'SET_TEMPO'))

        tempoChanges.sort(
            key=lambda item: item[0]
        ) #unlikely but possible that there are tempo events in separate tracks
        self.tempoChanges = tempoChanges
        self.ticksPerQuarterNote = midiObject.ticksPerQuarterNote

    def ticksToSeconds(self, n):
        # tempo is in microseconds per quarter note.
        newarr = list(self.tempoChanges)
        newarr = newarr + [(n, -1)] #add a final event, for processing conveniance
        ...
        for i in range(len(newarr)):
            ...
            totalTime += (nextTime - currentTime
                         ) * 1.0e-6 * float(currentTempo) / self.ticksPerQuarterNote
            if newarr[i + 1][1] == -1: #reached the time we are interested in.
                break
        return totalTime

    def getTrackLength(self, trackObj):
        if len(trackObj.events) == 0:
            return 0
        else:
            return self.ticksToSeconds(trackObj.events[-1].time)
...
def transposeMidi(midiObject, amt, tracknum='all'):
    #track can be number, like 0 1 2 3 or string 'all'
    ...
    if tracknum == 'all':
        for trackObject in midiObject.tracks:
            transposeTrack(trackObject, amt)
    else:
...
def getMidiExcerpt(midiObject, nTics): #note: is destructive, modifies things
    ...
    for track in midiObject.tracks:
        track.events = [
            evt for evt in track.events if not (
                evt.time < nTics and
                (evt.type == 'NOTE_ON' or evt.type == 'NOTE_OFF' or evt.type == 'PITCH_BEND')
            )
        ]
    ...
    def getKey(evt): #return tuple to be used as a key for latestMetaEvents.
        if evt.type == 'CONTROLLER_CHANGE':
            return (evt.channel, evt.type, evt.pitch)
        else:
            return (evt.channel, evt.type)
    ...
    for track in midiObject.tracks:
        for i in range(len(track.events)):
            ...
            if evt.type == 'END_OF_TRACK':
                track.events[i] = None
                continue #eliminate it
            if evt.time > nTics:
                evt.time -= (nTics - spaceForEvents)
            else:
                if evt.type != 'NOTE_ON' and evt.type != 'NOTE_OFF':
                    key = getKey(evt)
                    if key not in latestMetaEvents or (
                        key in latestMetaEvents and latestMetaEvents[key][1].time < evt.time
                    ):
...
def makeVeryRoughTimeExcerpt(midiObject, fPercentageDone):
    ...
    getMidiExcerpt(
        midiObject, int(fPercentageDone * lticks)
    ) #so if the tempo changes, this will be pretty wrong...
...
def restructureMidi(midiObject):
    newmidi = getMidiCopy(midiObject, deep=False)
    newmidi.tracks = [
        bmidilib.BMidiTrack() for i in range(17)
    ] #conductor track and then a track for all 16 channels
