...
class BaseMidiPlayer(object):
    ...
    def _saveTempMidiFile(self, objMidiFile):
        ...
        try:
            ...
            raise PlayMidiException('Could not create temporary file for midi playback.')
        f.close()
    ...
    def signalStop(self):
        ...
        if sys.platform == 'win32':
            ...
            handle = ctypes.windll.kernel32.OpenProcess(
                PROCESS_TERMINATE, False, self.process.pid
            )
...
class MciMidiPlayer(BaseMidiPlayer): #there should probably be only one instance of this...
    ...
    def playSynchronous(
        ...
        strFilename,
        fromMs=0
    ): #synchronous, waits for the song to be done. so you can't really use this for a long song unless you want to wait.
        if self.isPlaying:
            print('alreadyplaying')
            return
        self.isPlaying = True
    ...
    def playAsync(self, strFilename, fromMs=0): #should follow with a call to stop.
        ...
        self.isPlaying = True
        makeThread(
            self._playInThread, (strFilename, fromMs)
        ) #thread to automatically close when done.
...
class Mci(object):
    ...
    def send(self, command):
        ...
        if errorcode:
            txt = buffer
            raise PlayMidiException(
                'MCIError:' + str(self.get_error(errorcode)) + ':' + str(txt) + ':' +
                str(buffer)
            )
