...
class DlgResultsWindow(object):
    ...
    def _createIntermed(self):
        ...
        return notesinterpret.createIntermediateList(
            self.listQuantized, timesig, self.nQuantize, bTreble, bSharps
        )
    ...
    def openWindow(self, top):
        ...
        pack(
            Button(frame2, text='Save results as MusicXml', command=self.saveXml),
            side=LEFT,
            pady=25,
            padx=5
        )
        pack(
            Button(frame2, text='Save results as LilyPond', command=self.saveLily),
            side=LEFT,
            pady=25,
            padx=5
        )
        pack(
            Button(frame2, text='Preview Results', command=self.openPreview, default=ACTIVE),
            side=LEFT,
            pady=25,
            padx=5
        )
    ...
    def openPreview(self):
        ...
        opts = {}
        frScoreViewWithBtns = scoreview_intermed.ScoreViewWindow(
            top, intermed, bTreble, clefsfilepath, opts
        )
        top.focus_set()
...
if __name__ == '__main__':
    ...
    def test1():
        ...
        sampleList = [
            1, 2, 2, 4, 8, 4, 4, 8, 64, 64, 64, 64, 32, 32, 16, 16, 2, 4, 8, 8, 8, 8, 4, 4, 4,
            4, 8, 8, 8, 8
        ]
        noteList = []
        curtime = 0
        for n in sampleList:
            noteList.append(
                NotesRealtimeNoteEvent([60 + curtime % 7], curtime,
    ...
    def test2():
        ...
        for i in range(len(rlist)):
            ...
            if i % 2 == 1:
                pitch = 0 #a rest
            noteList.append(
                NotesRealtimeNoteEvent([pitch], curtime,
                                       curtime + old_div(4 * intermed.baseDivisions, n))
            )
            noteList[-1].isTied = False
            curtime += old_div(4 * intermed.baseDivisions, n)
        for i in range(len(rlist)):
            n = rlist[i]
            noteList.append(
                NotesRealtimeNoteEvent([70], curtime,
                                       curtime + old_div(4 * intermed.baseDivisions, n))
            )
