...
class NotesinterpretException(Exception):
    pass
...
class NotesRealtimeRecordedRaw(object):
    ...
    def getProcessedResults(self):
        ...
        if not len(listResults):
            return NotesinterpretException('Nothing was recorded.')

        #sort results by *start* time
        listResults.sort(key=lambda a: a[1])
        if listResults[0][0] != -1:
            return NotesinterpretException('must begin with pressing Tab.')
        if listResults[-1][0] != -1:
            return NotesinterpretException('must end with pressing Tab.')
        ...
        for tp in listResults:
            ...
            if pitch == -1:
                ...
                # a note event
                result.listNotes.append(
                    NotesRealtimeNoteEvent(pitch, start - startTime, end - startTime)
                )

        if not result.listPulses:
            return NotesinterpretException('no tabs!')
        return result
