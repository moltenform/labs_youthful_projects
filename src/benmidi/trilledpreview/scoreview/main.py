...
class App(object):
    ...
    def create_menubar(self, root):
        ...
        menuHelp.add_command(
            label='About',
            command=(
                lambda: scoreview_util.
                alert('ScoreView, by Ben Fisher 2009\n\n', 'benmidi ScoreView')
            )
        )
    ...
    def loadMidi(self, filename):
        ...
        def addLabel(text, y, x):
            ...
            if (x, y + 1) not in self.gridwidgets:
                smallFrame = Frame(
                    self.frameGrid, background='white', borderwidth=1, relief=RIDGE
                ) #GROOVE
        ...
        for rownum in range(len(self.objMidi.tracks)):
            ...
            if len(instarray) == 0:
                ...
                instname = str(
                    instarray[0]
                ) + ' (' + bmidilib.bmidiconstants.GM_instruments[instarray[0]] + ')'
            if channame == '10':
            ...
            if (rownum, 0) not in self.gridbuttons:
                btn = Button(
                    self.frameGrid,
                    text='Score',
                    command=scoreview_util.Callable(self.openScoreView, rownum)
                )
            ...
            if (rownum, 1) not in self.gridbuttons:
                btn = Button(
                    self.frameGrid,
                    text='List',
                    command=scoreview_util.Callable(self.openListView, rownum)
                )
