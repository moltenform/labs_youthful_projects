...
class ScoreViewFrame(Frame):
    ...
    def __init__(self, master, intermed, bTrebleClef, clefsfilepath, opts):
        ...
        if self.intermed.bSharps:
            self.prefersharpflat = '#'
        else:
            self.prefersharpflat = 'b'
        #don't draw the time sig for now...
        timesig = intermed.timesig
        assert timesig != None
    ...
    def scaleTicksToPixels(self, x): 
        #This is only used when finding where to place a note, but not used when placing sharp symbols /relative measurements
        return self.pixelsPerTick * float(x) + self.clefWidth

    def scaleYPositionToPixels(self, y): 
        #These coordinates are centered around the middle between the staffs. each unit is half of distance between two staff lines.
    ...
    def redraw(self):
        ...
        for note in self.intermed.noteList:
            if note.pitch == 0 or note.pitch[0] == 0:
                ...
                #take the first note of the group - we don't support drawing polyphony!
                pitch = note.pitch[0] 
                tielength = note.end - note.start
                if mapDurs[note.end - note.start] >= 16:
                    tielength += 32 #add some space per note
                self.drawNote(
                    currentPos, pitch, mapDurs[note.end - note.start], note.isTied, tielength
                )

            currentTime += note.end - note.start
            currentPos += note.end - note.start
            if mapDurs[note.end - note.start] >= 16:
                currentPos += 32 #add some space per note

            assert currentTime <= self.ticksPerMeasure
            if currentTime == self.ticksPerMeasure:
                self.drawbarline(currentPos)
                #some extra space for barline
                currentPos += old_div(self.intermed.baseDivisions, 8) 
                currentTime = 0
    ...
    def drawNote(self, currentPos, pitch, noteType, isTied, tieLength):
        ...
        #draw stem
        def drawstem(
            x, y, direction, noteType
        ): #direction is 1: upwards stem, direction is -1: downwards stem
            self.draw_line(x, y, x, y + direction * 7)
            # draw "tails" on fast notes
            if noteType >= 64:
                self.draw_line(x, y + direction * 4, x + 5, y + direction * 2)
            if noteType >= 32:
                self.draw_line(x, y + direction * 5, x + 5, y + direction * 3)
            if noteType >= 16:
                self.draw_line(x, y + direction * 6, x + 5, y + direction * 4)
            if noteType >= 8:
                self.draw_line(x, y + direction * 7, x + 5, y + direction * 5)

        if noteType >= 2:
            if self.bTrebleClef:
                drawstem(xPixels + 3, fy, 1, noteType) #treble clef, stem up
            else:
                drawstem(xPixels, fy, -1, noteType)
    ...
    def drawRest(self, currentPos, restType):
        ...
        if restType == 1 or restType == 2:
            ...
            #a simplification...
            if restType >= 64:
                self.draw_oval(xPixels + 2.0, startingY - 5.5, xPixels + 0.5, startingY - 6.5)
            if restType >= 32:
                self.draw_oval(xPixels + 3.0, startingY - 3.5, xPixels + 1.5, startingY - 4.5)
            if restType >= 16:
                self.draw_oval(xPixels + 4.0, startingY - 1.5, xPixels + 2.5, startingY - 2.5)
            if restType >= 8:
                self.draw_oval(xPixels + 5.0, startingY - .5, xPixels + 3.5, startingY + 0.5)
            self.draw_line(xPixels + 7.0, startingY + 0.5, xPixels + 4.0, startingY - 5.5)

    def getnoteposition(
        self, note, shift, prefersharpflat
