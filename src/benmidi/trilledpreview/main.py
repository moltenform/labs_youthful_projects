...
class Gui1(object):
    ...
    def __init__(self, root):
        ...
        pack(
            Label(frameMain, text='(This is a preview of a work-in-progress by Ben Fisher).'),
            side=TOP
        )
        pack(
            Label(frameMain, text='Try typing on your keyboard as if it were a piano:'),
            side=TOP
        )
        ...
        self.lblTransposition = pack(
            Label(frameMain, text='Transposition: +0   (lowest note is C4)'),
            side=TOP,
            anchor='w'
        )
        ...
        self.btnRecord = pack(
            Button(frame2, text='Record and notate...', command=self.startRecordProcess),
            side=LEFT,
            pady=25,
            padx=5
        )
        self.btnStop = pack(
            Button(frame2, text='Stop', command=self.stopRecord), side=LEFT, pady=25, padx=3
        )
        self.btnStop.config(state=DISABLED)
        pack(
            Label(frame2, text='\t Notate what is played, and export to Finale or Sibelius.'),
            side=LEFT
        )
