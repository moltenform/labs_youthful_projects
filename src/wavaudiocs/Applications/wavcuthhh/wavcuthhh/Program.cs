...
public class CWavcutMain
{
    public void Start(string[] args)
    {
        ...
        if (sWav == null || sTxt == null)
        {
            ...
            Console.WriteLine("\n\nWavcutHHH, by Ben Fisher, 2011");
            Console.WriteLine("WavcutHHH will split a large .wav file into several files based on a text file");
            Console.WriteLine("that specifies cut points.\n\n");
            Console.WriteLine("wavcut.exe input.wav input.txt\n");
            Console.WriteLine("First line of the .txt file is 'wavcut' or 'wavfindsilence'.");
...
class Program
{
    ...
    {
        Console.WriteLine("warning - we currently lose stereo data!!");
    ...
    class CTestPassThrough : IReadWaveData
    {
        CWaveWrite writer;
        ...
        public void ReceiveRawSample(byte s1a,byte s1b,byte s2a,byte s2b) { throw new WavCutException("no raw"); }
        public void ReceiveProcessedSample(double sa,double sb) {
            writer.WriteSample(sa,sb);
        }
        public void OnFinished() {  }
