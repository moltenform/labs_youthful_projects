...
def generate_tone(
    audiodata, fFreq, fSeconds, waveType='sine', amplitude=0.5
    ...
    if waveType == 'sine':
        ...
        audiodata.samples.extend(
            int((amplitude * math.sin(w * x)) + midval) for x in range(nSamples)
        )
    elif waveType == 'square':
        halfperiod = period / 2.0
        audiodata.samples.extend(
            int(midval - amplitude if (x % period > halfperiod) else midval + amplitude)
        ...
        audiodata.samples.extend(
            int(
                slope * (x % period) + start if (x % period < halfperiod) else -slope *
                (x % period) + stop
            ) for x in range(nSamples)
        )
...
def synthesize(audiodata, strInstrument, freq, length, amplitude=0.5):
    if strInstrument in ['sine', 'square', 'sawtooth', 'triangle', 'circle']:
        ...
        if strInstrument == 'whitenoise':
            ...
            audiodata.samples.extend(
                int(
                    audiodata.midval + random.random() * amplitude * .5 * audiodata.maxval * .5
            ...
            for i in range(nsamples):
                ...
                if val > audiodata.ceil[1]:
                    val = audiodata.ceil[1]
                if val < audiodata.ceil[0]:
                    val = audiodata.ceil[0]
                res[i] = int(val)
            ...
            manualsynth(
                audiodata, nsamples,
                [[freq, amplitude * 0.6], [freq * 1.0006079, amplitude * 0.4]]
            )
            ...
            manualsynth(
                audiodata, nsamples,
                _extend([[freq, .5 * 0.9**(8 - 1)]],
                        [[freq * (i + 1), .5 * 0.1 * 0.9**((8 - 1) - i)]
                         for i in range(1, 8)], [[freq * 1.00606, .5 * 0.9**(8 - 1)]],
                        [[freq * 1.00606 * (i + 1), .5 * 0.1 * 0.9**((8 - 1) - i)]
                         for i in range(1, 8)])
            )
        elif strInstrument == 'harmonics':
            manualsynth(
                audiodata, nsamples, [[freq, amplitude * 0.6], [freq * 2, amplitude * 0.2],
                                      [freq * 3, amplitude * 0.5]]
            )
        #experimental
...
def synth_squarephase(audiodata, fFreq, nSamples, amplitude=0.5):
    ...
    audiodata.samples.extend(
        int(
            midval - amplitude if (x % period > (x / float(nSamples) * period)) else midval +
            amplitude
        ) for x in range(nSamples)
    )
