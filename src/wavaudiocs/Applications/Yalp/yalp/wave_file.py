...
class WaveFile(object):
    ...
    def __init__(self, strFilename=None, nBits=16, nSampleRate=22050, nChannels=1):
        ...
        if strFilename != None:
            ...
            if f.getcomptype() != 'NONE':
                print('Compressed wave files not supported.')
        ...
        if nSampleRate == 44100:
            self.nSampleRate = 44100.
        elif nSampleRate == 22050:
            self.nSampleRate = 22050.
        else:
        ...
        if nChannels == 1:
            ...
            if nChannels == 2:
                print('Stereo not supported at this time...')
    ...
    def empty_copy(self):
        ...
        return WaveFile(
            nBits=self.sampleWidth * 8, nSampleRate=self.nSampleRate, nChannels=self.nChannels
        )
    ...
    def addwave(self, other, weight=0.5):
        ...
        if self.midval == 0:
            ...
            for i in range(len(othersamples)):
                mysamples[i] = int((mysamples[i] - self.midval) * oweight +
                                   (othersamples[i] - self.midval) * weight) + self.midval
