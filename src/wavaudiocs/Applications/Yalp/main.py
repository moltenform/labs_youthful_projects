...
class App(object):
    ...
    def add_menus(self, root):
        ...
        menuFile.add_command(
            label="New", command=self.new_audio, underline=0, accelerator='Ctrl+N'
        )
        menuFile.add_command(
            label="New 16bit 44", command=Callable(self.new_audio, 1), underline=3
        )
        menuFile.add_command(
            label="Open", command=self.open_audio, underline=0, accelerator='Ctrl+O'
        )
        menuFile.add_command(
            label="Save As", command=self.save_audio, underline=0, accelerator='Ctrl+S'
        )
        ...
        for effect in aeffects:
            if effect == '_':
                ...
                menuEffects.add_command(
                    label=effect[0], command=Callable(self.audio_fx, effect[1])
                )
    ...
    def manual_synth(self):
        ...
        w1 = [
            scaleAll * float(weight) / sumweights for weight in weights if weight != 0
        ] # weights are relative - 1,1,1,1 should be 0.25,0.25,0.25,0.25
        ...
        if self.fldFreq2.get() == '':
            ...
            w2 = [
                old_div(scaleAll * float(weight), sumweights2)
                for weight in weights2 if weight != 0
            ]
    ...
    def record_audio(self):
        ...
        wave_system.audio_record(
            'temp.wav', 3.0, self.audiodata.sampleWidth, self.audiodata.nSampleRate
        )
    ...
    def refresh(self):
        strText = 'Mono ' + str(self.audiodata.sampleWidth * 8) + 'bit, ' + str(
            self.audiodata.nSampleRate
        ) + 'Hz, ' + str(len(self.audiodata.samples) / float(self.audiodata.nSampleRate)) + 's'
    ...
    def refresh_picture(self):
        ...
        for i in range(nchunks):
            level = (self.audiodata.get_level(i * chunksize,
                                              (i + 1) * chunksize)) * (self.canvasH / 2.)
