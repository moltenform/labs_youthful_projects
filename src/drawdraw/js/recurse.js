...
{
	if (relativeShape.type!='lgen') {alerd('only linegen should produce next context.'); return;}
	outContext.startx = context.startx + context.length*relativeShape.lengthToStart*degcos(context.rotation+relativeShape.angleToStart)
	outContext.starty = context.starty + context.length*relativeShape.lengthToStart*degsin(context.rotation+relativeShape.angleToStart)
...
{	
	var outstartx = context.startx + context.length*relativeShape.lengthToStart*degcos(context.rotation+relativeShape.angleToStart)
	var outstarty = context.starty + context.length*relativeShape.lengthToStart*degsin(context.rotation+relativeShape.angleToStart)
	...
	if (relativeShape.type.startsWith('l')) //usually just lines are drawn, but allow linegen because of onsetCoords
	{
...
var lastDrawn = null; //todo: clean this up after more things
var contextPool = []; var ncontextPool = 0
function transform(contextQueue, relativeShapes, relativeGenerators, nThresholdBeforeDraw, nShapeLimit, adjustX)
{
	if (contextPool.length < nShapeLimit)
		while (contextPool.length<nShapeLimit)
			contextPool.push(new CContext())
	
	//fill up a pool. this will always waste some memory, but we won't hvae to realloc so much
	ncontextPool = 0
	...
	render_hideAllShapes() //hides ciirles
	renderAllLines(false); //hide earlier one
	if (relativeGenerators.length==0) {renderAllLines(false); return;} //pointless
	if (relativeShapes.length==0) {renderAllLines(false); return;} //pointless, avoid inf loop
	var nGeneration=0
	...
	while (true) {
		...
		for (var i=0; i<relativeShapes.length; i++) //we've already filtered out the invisible ones.
		{
			...
			if (nDrawn > g_nJustPerimeter)
			{
				...
				currentRawShape.x1 += adjustX; currentRawShape.x2 += adjustX
				
				if (currentRawShape.type=='c')
					renderCircle(currentRawShape)
				else
				{
				arResults.push('M'); arResults.push(currentRawShape.x1); arResults.push(currentRawShape.y1)
				arResults.push('L'); arResults.push(currentRawShape.x2); arResults.push(currentRawShape.y2)
				}
			}
			if (nDrawn> nShapeLimit)
			{renderAllLines(arResults); return;}
		}
		
		//add next contexts to the queue
		for (var i = 0; i<relativeGenerators.length; i++)
		{
			var nextcontext = new CContext() //todo: pull from pool instead
			//~ var nextcontext = contextPool[ncontextPool++]
...
//rawShape objects:
// { type:'l', visible:true (default), hasChildren:true (default), x1, y1, x2, y2 }
// { type:'c', visible:true (default), x1, y1, rx}
// type is mandatory.
function CRawShape(obj)
{
	...
	if (obj )
	{
		if (!obj.type ) { alerd('must pass type to rawShape constructor') }
		if ( !obj.x1 || !obj.y1) { alerd(debugprint(obj)); alerd('must pass cds to rawShape constructor') }
		for (var key in obj)
			this[key] = obj[key]
	}
}
...
{
	if (!rawShape.type.startsWith('l')) {alerd('can only be done for lines')}
