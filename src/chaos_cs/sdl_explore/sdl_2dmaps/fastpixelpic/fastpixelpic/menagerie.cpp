...
{
	if (TRUE) { //(!(g_settings->drawingOptions & maskOptionsDiagramColoring)) {
		//rainbow colors
		...
		if (val > (estimatedMax)*0.75)
		{
			...
			return SDL_MapRGB(pSurface->format,  255-(Uint8)(val*255),255-(Uint8)(val*255),(Uint8)255);
		}
		else
		{
			val = (val)/(estimatedMax*0.75);
			return SDL_MapRGB(pSurface->format, (Uint8) (val*255),(Uint8)(val*255),(Uint8)(val*255));
		}
...
#define LegendWidth 0
typedef struct { int whichHalf; SDL_Surface* pMSurface; CoordsDiagramStruct*diagram; FastMapsSettings * bounds; } PassToThread;
...
//	FastMapsSettings * boundsettings=p->bounds;
//
...
//	memcpy(&boundsettings, g_settings, sizeof(FastMapsSettings));
