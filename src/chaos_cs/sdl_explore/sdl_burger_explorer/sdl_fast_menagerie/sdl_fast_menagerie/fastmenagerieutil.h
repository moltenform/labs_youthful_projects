...
int alternateCountPhasePlotIsPosNeg(MenagFastSettings*settings,double c1, double c2, int whichThread)
{
	int CURRENTID =  whichThread? ++CURRENTID1 : ++CURRENTID2;
	int*arr = whichThread? arrThread1:arrThread2;
	double x,x_,y;
	...
	//drawing time.
	for (int i=0; i<100/2; i++) //see how changes if drawing increases?
	{
		x_ = c1*x - y*y; y= c2*y + x*y; x=x_;
		int px = lrint(PHASEW * ((x - X0) / (X1 - X0)));
		int py_times_256 = lrint(PHASEW*PHASEH * ((y - Y0) / (Y1 - Y0)));
...
theend:
	if (counted==0) return standardToColors((double)500, 1000);
	//else return standardToColors((double)0, 1000);
	int index=1000;
	if (hasbeenpos&&hasbeennegative) index= 100;
	if (hasbeennegative) index= 200;
	if (hasbeenpos) index= 300;

	return standardToColors((double)index, 1000);
}