...
def handle_keyboard_events(sheet, evt, dictEditableColumns):
    ...
    if (not sheet.GetGridCursorRow() and
        sheet.GetGridCursorRow() != 0) or sheet.GetGridCursorRow() < 0:
        ...
        if sheet.GetSelectedCells():
            ...
            if col in dictEditableColumns:
                ...
                for i in range(len(aClipboard)):
                    nRow = row + i
                    if nRow >= sheet.GetNumberRows():
                        break
                    sheet.SetCellValue(nRow, col, aClipboard[i])
                sheet.SelectBlock(
                    row, col, min(sheet.GetNumberRows() - 1, row + len(aClipboard) - 1), col
                )
