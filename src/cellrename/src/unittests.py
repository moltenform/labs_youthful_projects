...
def expectStringContains(s, sExpected):
    if not isString(s):
        raise RuntimeError('not even a string.')
...
def unittestsetup():
    ...
    for file in os.listdir(dir):
        if os.path.isdir(join(dir, file)):
            os.rmdir(join(dir, file))
        else:
            os.unlink(join(dir, file))
...
def engineunittest():
    ...
    expectEqual(
        renameFiles(dir, namestochange, ['pref' + name for name in namestochange]), True
    )
    for name in namestochange:
        assert exists(join(dir, 'pref' + name))
    expectEqual(
        renameFiles(dir, ['pref' + name for name in namestochange], namestochange), True
    )
    for name in namestochange:
    ...
    expectStringContains(
        renameFiles(dir, [names[0], names[1]], [names[0], names[1] + marker]), 'its name'
    )
    expectStringContains(
        renameFiles(dir, [names[0], names[1] + marker], [names[0], names[1]]), 'its name'
    )
    # expect failure, bad filenames
    expectStringContains(
        renameFiles(dir, ['notexist1', 'notexist2'], ['a', '']), 'invalid filename'
    )
    expectStringContains(
        renameFiles(dir, ['notexist1', 'notexist2'], ['a', 'ok:one']), 'invalid filename'
    )
    expectStringContains(
        renameFiles(dir, ['notexist1', 'notexist2'], ['a', 'some/other']), 'invalid filename'
    )
    expectStringContains(
        renameFiles(dir, ['notexist1', 'notexist2'], ['a', 'some\\other']), 'invalid filename'
    )
    ...
    expectStringContains(
        renameFiles(dir, ['6', '7', '8', '9'], ['y', 'x', 'z', 'x']), 'Duplicate'
    )
    if os.name == 'nt':
        expectStringContains(
            renameFiles(dir, ['6', '7', '8', '9'], ['y', 'X', 'z', 'x']), 'Duplicate'
    ...
    expectStringContains(
        renameFiles(dir, [names[0], names[1]], ['existdir', names[1]]), 'exists'
    )
    ...
    if os.name == 'nt':
        ...
        expectStringContains(
            renameFiles(dir, [names[0], names[1]], [names[0], names[8].upper()]), 'exists'
        )

    # expect failure, cannot access
    expectStringContains(
        renameFiles(dir, ['notexist1', 'notexist2'], ['a', 'b']), 'longer exists'
    )
    expectStringContains(
        renameFiles(dir, [names[0], 'notexist1', 'notexist2'], [names[0], 'a', 'b']),
        'longer exists'
    )
    if os.name == 'nt':
...
def dataunittest_files():
    ...
    testSort1.sort('size')
    expectEqual(
        testSort1.data[0].filename in ('directory1', 'directory.jpg'), True
    ) # directories always at top
    expectEqual(
        testSort1.data[1].filename in ('directory1', 'directory.jpg'), True
    ) # directories always at top
...
def dataunittest_transforms():
    ...
    expectEqual(
        tostring(test1), 'a1a1 aaaaaa|a2a2 bbbbbb.jpg|a3a3 cc.okcc.okcc.Ok.jpg|a4a4 dddddd.a'
    )
    useFakeData(test1, testNames1)
    ...
    test1.transformReplace(
        '.jpg', '.png'
    ) # unlike the rest, this *should* be able to rename extensions
    ...
    expectEqual(
        tostring(test2), 'a_a|test_athat|first,second|xx,yy,zz|cc.Ok.jpg|ccnOk.jpg|dd.a'
    )
    useFakeData(test2, testNames2)
    test2.transformRegexReplace('(a', '_a', False, True) # not regex, case sensitive
    expectEqual(
        tostring(test2), 'a_a|test(Athat|first,second|xx,yy,zz|cc.Ok.jpg|ccnOk.jpg|dd.a'
    )
    useFakeData(test2, testNames2)
    test2.transformRegexReplace('cc.Ok', 'match', False, False) # not regex, case sensitive
    expectEqual(
        tostring(test2), 'a(a|test(Athat|first,second|xx,yy,zz|match.jpg|ccnOk.jpg|dd.a'
    )
    useFakeData(test2, testNames2)
    test2.transformRegexReplace('cc.Ok', 'match', True, False) # is regex, case sensitive
    expectEqual(
        tostring(test2), 'a(a|test(Athat|first,second|xx,yy,zz|match.jpg|match.jpg|dd.a'
    )
    useFakeData(test2, testNames2)
    test2.transformRegexReplace(
        r'(\w+),(\w+)', r'\2-\1', True, False
    ) # is regex, case sensitive
    expectEqual(
        tostring(test2), 'a(a|test(Athat|second-first|yy-xx,zz|cc.Ok.jpg|ccnOk.jpg|dd.a'
    )
