...
class PythonPixelsApp(object):
    ...
    def __init__(self, root):
        ...
        self.fldScript = pack(
            ScrolledText.ScrolledText(frameBtns, height=6, width=100, wrap=NONE),
            side=TOP,
            fill=BOTH,
            expand=True
        )
        self.fldScript.insert(
            END, '#Flip red and green\nloop:\n\tR=g\n\tG=r-30\nprint("this is Python code!")'
        )
        ...
        self.fldOut = pack(
            ScrolledText.ScrolledText(frameBtns, height=4, width=100, wrap=NONE),
            side=TOP,
            fill=X
        )
    ...
    def makeFavoritesMenu(self):
        ...
        astrFolders = [
            strFolder for strFolder in astrFolders
            if tkutil.isDirectory(os.path.join('scripts', strFolder)) and
            not strFolder.startswith('.')
        ]
        ...
        for strFolder in astrFolders:
            ...
            for strItem in astrFiles:
                submenu.add(
                    'command',
                    label=strItem,
                    command=tkutil.Callable(self.menu_pickFavorite, strFolder, strItem)
                )
    ...
    def menu_saveImage(self):
        if self.imgOutput == None:
            ...
            return
        filename = tkutil_dialog.ask_savefile(
            title='Save Image',
            types=['.png|Png image', '.bmp|Bitmap image', '.jpg|Jpeg image', '.tif|TIFF image']
        )
        if not filename:
            return
    ...
    def menu_batch(self):
        ...
        confirm = tkutil_dialog.ask_okcancel(
            'Run current script on ' + str(len(files)) + ' files of type .' + ext +
            ' in directory ' + path + '?'
        )
        if not confirm:
            return
