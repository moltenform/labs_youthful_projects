    ...
    for construct in constructs:
        ...
        if nLoops > 1:
            ...
            if construct == 'loop:':
                ...
                for j in range(startLoop + 1, endLoop + 1):
                    slines[j] = '    ' + slines[j]
                loopPrefix, loopSuffix = createLoopHue(
                    construct, slines[startLoop + 1:endLoop + 1]
                )
...
def pullConstruct(slines, construct):
    ...
    for i in range(len(slines)):
        if slines[i] == construct:
            ...
            i += 1
            while i < len(slines):
                if (not (slines[i].startswith('\t') or
                         slines[i].startswith('  '))) and slines[i].strip() != '':
                    endLoop = i - 1
                    break
                i += 1
            if endLoop == None:
                endLoop = len(slines) - 1
            break
...
for y in range(height):
    ...
    startLine = '\n' + ' ' * 8
    if 'r' in occurred:
        sPrefix += startLine + 'r = ra[x,y]'
    if 'g' in occurred:
        sPrefix += startLine + 'g = ga[x,y]'
    if 'b' in occurred:
        sPrefix += startLine + 'b = ba[x,y]'

    # note: in recent PIL, if you do not cast to int, you get
    # the error: "new style getargs format but argument is not a tuple"
    if 'R' in occurred:
        sSuffix += startLine + 'ra[x,y] = int(max(min(R,255),0))'
    if 'G' in occurred:
        sSuffix += startLine + 'ga[x,y] = int(max(min(G,255),0))'
    if 'B' in occurred:
        sSuffix += startLine + 'ba[x,y] = int(max(min(B,255),0))'
...
for y in range(height):
    ...
    if space == 'loop_hsv:':
        if occurred:
            sPrefix += startline + 'h,s,v=H,S,V=rgb_to_hsv(ra[x,y]/256.0,ga[x,y]/256.0,ba[x,y]/256.0)'
        ...
        if occurred:
            sPrefix += startline + 'h,l,s=H,L,S=rgb_to_hls(ra[x,y]/256.0,ga[x,y]/256.0,ba[x,y]/256.0)'
...
def createMap(astr):
    ...
    for line in astr:
        if line.startswith('R'):
            if getTokens(['g', 'b'], line):
                raise 'Maps cannot depend on other colors. Use loop: instead.'
        elif line.startswith('G'):
            if getTokens(['r', 'b'], line):
                raise 'Maps cannot depend on other colors. Use loop: instead.'
        elif line.startswith('B'):
            if getTokens(['r', 'g'], line):
                raise 'Maps cannot depend on other colors. Use loop: instead.'
...
for iii in range(256): 
    ...
    startline = '\n' + ' ' * 4
    if 'R' in occurred:
        sSuffix += startline + 'rmap[iii] = max(min(R,255),0)'
    if 'G' in occurred:
        sSuffix += startline + 'gmap[iii] = max(min(G,255),0)'
    if 'B' in occurred:
        sSuffix += startline + 'bmap[iii] = max(min(B,255),0)'
    sSuffix += '''
table = rmap
table.extend(gmap)
table.extend(bmap)
...
def runCode(source, imgInput):
    ...
    dlocals['rim'], dlocals['gim'], dlocals[
        'bim'] = rim, gim, bim # I don't expect this to be used often

    #expose modules
    dlocals['Image'] = Image
    dlocals['ImageChops'] = ImageChops
    dlocals['ImageEnhance'] = ImageEnhance
    dlocals['ImageFilter'] = ImageFilter
    dlocals['ImageOps'] = ImageOps
